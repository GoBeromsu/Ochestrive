...
io.on("connection", (socket) => {
    ...
    socket.on("message", function (message) {
        console.log(
            "Connection: " + socket.id + " receive message: " + message.id
        );
        ...
        switch (message.id) {
            ...
            case "receiveVideoFrom":
                console.log(
                    socket.id + " receiveVideoFrom : " + message.sender
                );
                receiveVideoFrom(
                    socket,
                    message.sender,
    ...
    if (iceCandidateQueue) {
        while (iceCandidateQueue.length) {
            ...
            console.error(
                "user : " +
                    userSession.id +
                    " collect candidate for outgoing media"
            );
...
function receiveVideoFrom(socket, senderId, sdpOffer, callback) {
    ...
    getEndpointForUser(userSession, sender, function (error, endpoint) {
        ...
        endpoint.processOffer(sdpOffer, function (error, sdpAnswer) {
            console.log(
                "process offer from : " + senderId + " to " + userSession.id
            );
            if (error) {
                return callback(error);
            }
        ...
        getRoom(userSession.roomName, function (error, room) {
            ...
            room.pipeline.create("WebRtcEndpoint", (error, incomingMedia) => {
                ...
                var iceCandidateQueue =
                    userSession.iceCandidateQueue[sender.id];
                if (iceCandidateQueue) {
                    while (iceCandidateQueue.length) {
                        var message = iceCandidateQueue.shift();
                        console.log(
